...
class TeamService:
    ...
    @staticmethod
    async def create_team(form: TeamCreateForm, creator: CurrentUser) -> Team:
    ...
    @staticmethod
    async def my_teams(user: CurrentUser) -> list[Team]:
    ...
    async def add_team_member(
        ...
        await TeamService.require_team_role(
            team_id, user.id, [TeamMemberRole.CREATOR, TeamMemberRole.ADMIN]
        )

        member = await UserService.get_user_by_email(member_email)
    ...
    async def require_team_role(
        team_id: ObjectId, member_id: ObjectId, allowed_roles: list[TeamMemberRole]
    ):
